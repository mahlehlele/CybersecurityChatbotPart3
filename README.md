...
1. Running WPF GUI.
2. Task assistant showing a task saved from `tasks.json`.
3. Quiz question with feedback.
4. Activity log showing recent actions.
5. GitHub Actions green tick.

## Video presentation checklist

Your video should show:

- Your own voice introduction and student number.
- GUI launch, voice greeting, and ASCII art.
- Part 2 keyword recognition, random responses, memory, sentiment, and follow-up flow.
- Task assistant adding, reading, completing, and deleting tasks.
- The `tasks.json` file changing after task actions.
- Quiz with immediate feedback and final score.
- NLP examples using three different phrasings.
- Activity log with latest 10 entries and Show More.
- Code explanation for `TaskStorageHelper.cs`, `TaskManager.cs`, `QuizManager.cs`, `ActivityLogger.cs`, `ChatBot.cs`, and `KeywordResponder.cs`.
- GitHub commits, three releases, and green GitHub Actions run.
